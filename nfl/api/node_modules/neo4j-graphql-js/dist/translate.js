...
var relationFieldOnNodeType = exports.relationFieldOnNodeType = function relationFieldOnNodeType(_ref2) {
  ...
  return (0, _extends3.default)({
    initial: '' + initial + fieldName + ': ' + (!(0, _utils.isArrayType)(fieldType) ? 'head(' : '') + '[(' + (0, _utils.safeVar)(variableName) + ')' + (relDirection === 'in' || relDirection === 'IN' ? '<' : '') + '-[:' + (0, _utils.safeLabel)(relType) + ']-' + (relDirection === 'out' || relDirection === 'OUT' ? '>' : '') + '(' + (0, _utils.safeVar)(nestedVariable) + ':' + (0, _utils.safeLabel)(isInlineFragment ? interfaceLabel : innerSchemaType.name) + queryParams + ')' + (temporalClauses.length > 0 ? 'WHERE ' + temporalClauses.join(' AND ') : '') + ' | ' + nestedVariable + ' {' + (isInlineFragment ? 'FRAGMENT_TYPE: "' + interfaceLabel + '",' + subSelection[0] : subSelection[0]) + '}]' + (!(0, _utils.isArrayType)(fieldType) ? ')' : '') + skipLimit + ' ' + commaIfTail
  }, tailParams);
};
...
var translateQuery = exports.translateQuery = function translateQuery(_ref9) {
  ...
  var _filterNullParams = (0, _utils.filterNullParams)({ offset: offset, first: first, otherParams: otherParams }),
...
var nodeQuery = function nodeQuery(_ref11) {
  ...
  var predicateClauses = [idWherePredicate].concat((0, _toConsumableArray3.default)(nullFieldPredicates), (0, _toConsumableArray3.default)(temporalClauses)).filter(function (predicate) {
    return !!predicate;
  }).join(' AND ');
...
var relationshipCreate = function relationshipCreate(_ref17) {
  ...
  params = (0, _extends3.default)({}, preparedParams, subParams);
  var query = '\n      MATCH (' + fromVariable + ':' + fromLabel + ' ' + (fromTemporalClauses && fromTemporalClauses.length > 0
